...
def cmp_dm_img_tri2D(i_file_name,i_th):
    ...
    vert = np.loadtxt("dataset/"+i_file_name+'/vert_list.txt')
    vert[:, 2] = vert[:, 2] / np.max(vert[:, 2])
    edge = np.loadtxt("dataset/" + i_file_name + '/edge_list.txt')
    triangle = np.loadtxt("dataset/" + i_file_name + '/tri_list.txt')
