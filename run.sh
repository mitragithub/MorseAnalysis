...
python get_pixels.py $image_path $output_folder
...
python triangulation.py $image_path $input_folder $GAUSSIAN

# Run discrete morse with vert, edge, tri files
echo "Running discrete morse......"
python discrete_morse.py $filename -t 2 $persistence_threshold

# Remove redundant edges of discrete morse output, in new_vert, new_edge.txt
echo "Running RemoveBranch2D......"
./simplify_by_intensity $output_folder $l $w $intensity_threshold
...
python graph2geojson.py $output_folder
